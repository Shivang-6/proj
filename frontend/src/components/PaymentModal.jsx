...
// New CartPaymentModal for cart payments
export const CartPaymentModal = ({ isOpen, onClose, cartItems, user, onPaymentSuccess }) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [orderData, setOrderData] = useState(null);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const options = {
        ...
        notes: {
          address: 'CampusKart Cart Purchase'
        },
        theme: {
          color: '#3B82F6'
        },
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Complete Cart Purchase</h2>
        ...
        {loading && !orderData ? (
          ...
          <div className="text-center py-8">
            <div className="text-red-500 text-lg mb-2">⚠️</div>
            <p className="text-red-600 mb-4">{error}</p>
            <button onClick={createOrder} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">Try Again</button>
          </div>
        ) : (
          <>
            {/* Cart Summary */}
            <div className="bg-gray-50 rounded-lg p-4 mb-4 max-h-40 overflow-y-auto">
              {cartItems.map(item => (
                <div key={item._id} className="flex items-center space-x-3 mb-2">
                  <img src={item.imageUrls?.[0] || item.imageUrl || 'https://placehold.co/40x40?text=No+Img'} alt={item.name} className="w-10 h-10 rounded object-cover" />
                  <div className="flex-1">
                    <h3 className="font-semibold text-gray-800 text-sm">{item.name}</h3>
                    <p className="text-xs text-gray-600">Qty: {item.quantity}</p>
                  </div>
                  <span className="text-green-600 font-bold text-sm">₹{item.price * item.quantity}</span>
                </div>
              ))}
            </div>
            {/* Payment Details */}
            ...
            {/* Action Buttons */}
            <div className="space-y-3">
              <button onClick={handlePayment} disabled={loading} className="w-full bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                {loading ? (
                  <>
                    <div className="inline-block animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                    Processing...
              ...
              <button onClick={onClose} className="w-full bg-gray-200 text-gray-800 py-3 rounded-lg hover:bg-gray-300 transition">Cancel</button>
            </div>
            <div className="mt-4 text-center">
